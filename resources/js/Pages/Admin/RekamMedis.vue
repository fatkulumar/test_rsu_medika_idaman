...
function showModal(rekamMedisId: String) {
    form.rekam_medis_id = rekamMedisId
    modal.value = true
}
...
function hanldeDelete(id: String, name: String) {
    try {
        ...
        if (konfirm) {
            router.delete(`/admin/rekam-medis/${id}`)
        }
    } catch (error)  {
        console.log(error)
    }
